...
for package in $vesta_software; do
    ...
    # Download the package from the focal repository
    wget -q "http://$RHOST/focal/pool/vesta/$package"_*.deb -O "$package.deb"
    if [ $? -ne 0 ]; then
        echo "Failed to download $package, trying alternative URL..."
        wget -q "http://$RHOST/focal/pool/main/$package"_*.deb -O "$package.deb"
    fi
